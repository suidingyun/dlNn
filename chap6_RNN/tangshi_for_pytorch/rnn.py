...
class RNN_model(nn.Module):
    ...
    def forward(self,sentence,is_test = False):
        ...
        # ???
        hidden = Variable(torch.zeros(2,batch_input.size(0),self.lstm_dim, device=batch_input.device))
        cell = Variable(torch.zeros(2,batch_input.size(0),self.lstm_dim, device=batch_input.device))
